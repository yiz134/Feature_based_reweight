...
def reweight_feature(self, weight, X_tr, y_tr, X_val, y_val,
                     alpha0=1, recompute=False, ratio=None, correction=None, max_clip=1, visualize=False):
    ...
    Kmat, y_val, Kmat_raw = transform_Kmat(Kmat, y_val)
    ...
    if correction is not None:
        y_tr = label_correction(Kmat, y_tr, y_val, w_direction, correction_rate=correction)
        match = (y_tr[:, None] == y_val[None, :])  # (N_tr, N_val), dtype=bool
        sign_mask = torch.where(match,
                                torch.tensor(1.0, device=Kmat.device),
                                torch.tensor(-alpha0, device=Kmat.device))  # (N_tr, N_val)
    ...
    if recompute:
        weight = torch.ones_like(weight, device=weight.device) / 2#(0.5*len(weight))
